...
async function fetchWeatherData(lat, lon) {
    try {
        ...
        weatherData.innerHTML = `
            <p>Temperature: ${CONFIG.FALLBACK_VALUES.temperature}°C (estimated)</p>
            <p>Humidity: ${CONFIG.FALLBACK_VALUES.humidity}% (estimated)</p>
...
async function fetchCarbonData(lat, lon) {
    try {
        const response = await fetch(
            ...
            {
                headers: {
                    'auth-token': CONFIG.ELECTRICITYMAP_API_KEY
                }
            }
        );
...
function useFallbackCarbonValues() {
    const fallbackIntensity = CONFIG.FALLBACK_VALUES.carbonIntensity;
    carbonData.innerHTML = `
